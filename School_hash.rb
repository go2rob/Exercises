...
best_teacher = school_hash.group_by{|h| h[:teacher]}.map { |k,v| v.collect { |h| (h[:mark1].to_i + h[:mark2].to_i)/2.0 } }
teachers.zip(best_teacher.collect{ |array| (array.reduce(:+).to_f / array.size).round(2)}).max_by{|a| a[1]}